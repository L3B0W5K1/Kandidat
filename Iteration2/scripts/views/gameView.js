...
class GameView {
    ...
    this.ctx.beginPath();
    this.ctx.moveTo(x1, y1);
    this.ctx.lineTo(fstControl.node.posX - CONTROL_RADIUS * Math.cos(angle), fstControl.node.posY - CONTROL_RADIUS * Math.sin(angle));
    this.ctx.stroke();
    this.ctx.closePath();
    }
    ...
    rendercontrolNodes(controlNodes) {
     ...
     // Render control as a circle
        this.ctx.beginPath();
        this.ctx.arc(node.node.posX, node.node.posY, CONTROL_RADIUS, 0, Math.PI * 2);
    ...
    // Render control number
      ...
      if (controlNodes[parseInt(controlNum) + 1]) {
        ...
        const angle = Math.atan2(nextNode.posY - node.node.posY, nextNode.posX - node.node.posX);
        ...
        this.ctx.beginPath();
        this.ctx.arc(node.node.posX, node.node.posY, CONTROL_RADIUS-3, 0, Math.PI * 2);
